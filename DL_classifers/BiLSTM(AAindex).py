...
def train(model, train_loader,valid_loader,device):
    ...
    with torch.no_grad():
        ...
        np.save('../np_weights/BiLSTM(AAindex)_y_train.npy',y_true)
        np.save('../np_weights/BiLSTM(AAindex)_y_train_score.npy',y_score)
        # 计算SN，SP，ACC，MCC
...
np.save('../np_weights/BiLSTM(AAindex)_roc_auc.npy', roc_auc)
...
def Kf_show(plt, base_fpr, roc, roc_auc):
    ...
    plt.savefig('../figures/BiLSTM(AAindex).png')
    plt.show()
