...
def train(model, train_loader,valid_loader,device):
    ...
    with torch.no_grad():
        ...
        np.save('../np_weights/LSTM(BLOSUM62)_y_train.npy',y_true)
        np.save('../np_weights/LSTM(BLOSUM62)_y_train_score.npy',y_score)
        # SN，SP，ACC，MCC
...
for train_index, valid_index in kf.split(train_set):
    ...
    #save model
    torch.save(model.state_dict(), '../DL_weights/'+str(fold) + '_LSTM(BLOSUM62)_kfold_model.pth'.format(fold))

    fold += 1


np.save('../np_weights/LSTM(BLOSUM62)_roc_auc.npy', roc_auc)
np.save('../np_weights/LSTM(BLOSUM62)_roc.npy', roc)
...
np.save('../np_weights/LSTM(BLOSUM62)_SN_SP_ACC_MCC.npy', kfold_SN_SP_ACC_MCC)
...
def Kf_show(plt, base_fpr, roc, roc_auc):
    ...
    plt.savefig('../figures/LSTM(BLOSUM62).png')
    plt.show()
...
def total_train(model, train_loader,device):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) == num_epochs:
            # save model
            torch.save(model.state_dict(), '../DL_weights/LSTM(BLOSUM62)-FinalWeight.pth')
