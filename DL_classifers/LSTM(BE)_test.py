...
#load mmodel
model_path='../DL_weights/LSTM(BE)-FinalWeight.pth'
...
with torch.no_grad():
    ...
    test_true = np.concatenate(y_test_true)
    test_score = np.concatenate(y_test_score)

    np.save('../np_weights/LSTM(BE)_y_test_true.npy',test_true)
    np.save('../np_weights/LSTM(BE)_y_test_score.npy',test_score)

    fpr, tpr, _ = metrics.roc_curve(test_true, test_score)
    ...
    np.save('../np_weights/LSTM(BE)_eval.npy',eval_SN_SP_ACC_MCC)
