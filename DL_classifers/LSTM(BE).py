...
class Model_LSTM(nn.Module):
    ...
    def forward(self, inputs):
       ...
       # LSTM layer
        ...
        Bilstm_outputs = self.dropout1(lstm_outputs)
...
def train(model, train_loader,valid_loader,device):
    ...
    with torch.no_grad():
        ...
        np.save('../np_weights/LSTM(BE)_y_train.npy',y_true)
        np.save('../np_weights/LSTM(BE)_y_train_score.npy',y_score)
        # 计算SN，SP，ACC，MCC
...
for train_index, valid_index in kf.split(train_set):
    ...
    # 保存模型：
    torch.save(model.state_dict(), '../DL_weights/'+str(fold) + '_LSTM(BE)_kfold_model.pth'.format(fold))

    fold += 1


np.save('../np_weights/LSTM(BE)_roc_auc.npy', roc_auc)
np.save('../np_weights/LSTM(BE)_roc.npy', roc)
...
#平均的五倍交叉验证结果：
np.save('../np_weights/LSTM(BE)_SN_SP_ACC_MCC.npy', kfold_SN_SP_ACC_MCC)
...
def Kf_show(plt, base_fpr, roc, roc_auc):
    ...
    plt.savefig('../figures/LSTM(BE).png')
    plt.show()
...
def total_train(model, train_loader, device):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) == num_epochs:
            # 保存模型：
            torch.save(model.state_dict(), '../DL_weights/LSTM(BE)-FinalWeight.pth')
